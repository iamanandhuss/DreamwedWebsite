...
// 1. Update public/app.js to apply screenshot titles, prices, descriptions, and cover photos
...
if (idx === -1) {
    ...
    window.restructurePackageCards = function() {
        ...
        console.log("Updated app.js code block successfully.");
    }
}
fs.writeFileSync(appJsPath, js, 'utf8');

// 2. Append list items text color override to public/style.css to resolve white-on-white text issues
const styleCssPath = path.join(__dirname, '../public/style.css');
let css = fs.readFileSync(styleCssPath, 'utf8');
css = css.replace(/\r\n/g, '\n');

const listOverrideStyle = `
/* Override checklist item styles inside white details modal to make them dark and readable */
.pkg-details-modal-checklist-container .package-features li {
    color: #27272a !important;
    font-size: 13.5px !important;
}

.pkg-details-modal-checklist-container .package-features li i {
    color: #16a34a !important;
    font-size: 14px !important;
}

.pkg-details-modal-checklist-container .package-event-section {
    border-color: #e4e4e7 !important;
}
`;

// Only append if not already present
if (!css.includes('.pkg-details-modal-checklist-container .package-features li')) {
    css += '\n' + listOverrideStyle;
    console.log("Successfully appended checklist color overrides to style.css");
}
