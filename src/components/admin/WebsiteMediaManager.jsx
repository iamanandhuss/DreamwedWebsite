...
import { 
  ...
  Camera, FolderOpen, X, Search, CheckCheck
} from "lucide-react";
...
const WebsiteMediaManager = () => {
  ...
  const [activeSectionTab, setActiveSectionTab] = useState("portraits"); // hero | bento | portraits | services | all_media
  ...
  // Filtered list for media library modal
  ...
  // Reusable Slot Card Component with Direct Change / Upload / Library picker
        ...
        <div className={`${aspect} rounded-2xl overflow-hidden bg-black relative border border-zinc-800 shadow-md`}>
          ...
          {isThisSlotUploading && (
            <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center gap-2 p-4 text-center z-20">
              ...
              <span className="text-[11px] text-white font-bold">Uploading & Replacing...</span>
            </div>
          )}
    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="p-6 md:p-8 rounded-[28px] bg-gradient-to-r from-zinc-900 via-zinc-900/90 to-zinc-950 border border-zinc-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 shadow-2xl">
        <div className="space-y-2">
          ...
          <p className="text-xs md:text-sm text-zinc-400 max-w-2xl leading-relaxed font-light">
            Click <strong>Upload</strong> or <strong>Choose</strong> on any card to replace images live across the website.
          </p>
        </div>
        ...
        <div className="max-w-md mx-auto space-y-4">
          ...
          <div>
            <h3 className="text-lg font-bold text-white mb-1">
              {uploading && !uploadingSlot ? "Uploading to Cloudinary CDN..." : "Upload New Website Photos"}
            </h3>
            <p className="text-xs text-zinc-400 font-light">
              Drag & drop photos here to add them to your Cloudinary Media Library
            </p>
          </div>
      ...
      {activeSectionTab === "services" && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
              <span>📸</span> Photography Packages & Stories (5 Slots)
            </h3>
          </div>
