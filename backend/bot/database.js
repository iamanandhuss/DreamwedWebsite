...
function saveBooking(booking) {
  ...
  const newBooking = {
    ...
    customer_address_2: booking.customer_address_2 || '',
...
function saveGallery(gallery) {
  ...
  if (existingIndex > -1) {
    ...
    const updated = {
      ...
      weddingDate: gallery.weddingDate !== undefined ? gallery.weddingDate : (existing.weddingDate || ""),
      location: gallery.location !== undefined ? gallery.location : (existing.location || ""),
      ...
      theme: gallery.theme || existing.theme || "editorial",
      accessCode: gallery.accessCode || existing.accessCode,
      selectionCode: gallery.selectionCode !== undefined ? gallery.selectionCode : (existing.selectionCode || existing.accessCode || ""),
      guestCode: gallery.guestCode !== undefined ? gallery.guestCode : (existing.guestCode || existing.accessCode || ""),
      brideCode: gallery.brideCode !== undefined ? gallery.brideCode : (existing.brideCode || ""),
      groomCode: gallery.groomCode !== undefined ? gallery.groomCode : (existing.groomCode || ""),
      loginMode: gallery.loginMode || existing.loginMode || "two_code_mode",
      storyData: gallery.storyData !== undefined ? gallery.storyData : (existing.storyData || null),
      photos: gallery.photos && gallery.photos.length > 0 ? gallery.photos : (existing.photos || []),
      updated_at: getDbDate()
    };
    ...
    const newGallery = {
      ...
      theme: gallery.theme || "editorial",
      accessCode,
      selectionCode: gallery.selectionCode || `SELECT-${accessCode}`,
      guestCode: gallery.guestCode || `GUEST-${accessCode}`,
      brideCode: gallery.brideCode || `BRIDE-${accessCode}`,
      groomCode: gallery.groomCode || `GROOM-${accessCode}`,
      loginMode: gallery.loginMode || "two_code_mode",
      storyData: gallery.storyData || null,
